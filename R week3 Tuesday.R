...
#Recode the age_cat to make it ordinal and factored
age_cat <-factor(managers$AgeCat,
                 order = TRUE, levels = c("Young", "Middle Aged", "Elder"))
...
#create a new col summary_col
#that contains summary of each row and then create a new col called
#mean_value to contain mean of all answers
...
# Add to managers
#overwriting the old managers
#datafram with a new dataframe called the name name

managers <- data.frame(managers, summary_col)
managers

#create a new col called mean_value to contain means of the answers

mean_value <- rowMeans(managers[ ,c(5,6,7,8,9)], na.rm = TRUE)
managers <- data.frame(managers, mean_value)
managers
#rename summary_ col as summary and mean_value as mean

#names(managers)[names(managers) == "summary_col"] <- "summary"
#names(managers)[names(managers) == "mean_value"] <- "mean"
